...
var (
	...
	updateMode = flag.Int("update-mode", int(mysqlop.ModeBatchUpdateTxn), "update mode")
)
...
func main() {
	...
	opts := []mysqlop.NewExecutorOption{
		mysqlop.WithUpdateMode(mysqlop.UpdateMode(*updateMode)),
	}
	exec, err := mysqlop.NewExecutor(ctx, *dsn, *batchSize, *workerSize, *maxPending, opts...)
	if err != nil {
